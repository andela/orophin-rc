...
import isGuestUser from "../../../../included/product-detail-simple/client/util/isGuest";

class ShopRatingsComponent extends Component {
  static propTypes = {
  ...
  postRatings = (_ref) => {
    ...
    if (type === "click") {
      const rateeId = Reaction.getShopId();
      const userId = Meteor.userId();
      Meteor.call("insert/ShopRatings", {
        userId,
        rateeId,
        ratingsScore: rating
      }, (error) => {
  ...
  updateRatings = (_ref) => {
    ...
    if (type === "click") {
      ...
      Meteor.call("update/ShopRatings",
        id,
        rating
        , (error) => {
          if (error) {
...
const composer = (props, onData) => {
  const userId = Meteor.userId();
  const rateeId = Reaction.getShopId();
  const subscription = Meteor.subscribe("ShopRatings", rateeId);
  let hasRated = false;
  let averageRating = 0;

  if (subscription.ready()) {
    let total = 0;
    const ratings = ShopRatings.find({ rateeId }).fetch();
    const userRatings = ShopRatings.find({ userId, rateeId }).fetch();
...
registerComponent("ShopRatingsComponent", ShopRatingsComponent, composeWithTracker(composer));

export default composeWithTracker(composer)(ShopRatingsComponent);
