...
const composer = (props, onData) => {
  ...
  const subscription = Meteor.subscribe("ProductRatings", productId);
  let hasRated = false;
  let averageRating = 0;

  if (subscription.ready()) {
    let total = 0;
    const ratings = ProductRatings.find({ productId }).fetch();
    const userRatings = ProductRatings.find({ userId, productId }).fetch();
